...
#define WIFI_SSID                   "Starlink Nasa Jampa"
#define WIFI_PASS                   "Nasa@2024"
...
{
    ESP_LOGI(TAG, "wifi_init_sta initing...");
    ...
    if (bits & WIFI_CONNECTED_BIT) ESP_LOGI(TAG, "connected to ap SSID:%s password:%s", WIFI_SSID, WIFI_PASS);
    else if (bits & WIFI_FAIL_BIT) ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s", WIFI_SSID, WIFI_PASS);
